...
import { FarmIcon, CoinIcon, WheatIcon, WindmillIcon, FlourIcon } from '../ui/icons';
...
export function LeftSideBar() {
  ...
  const { moneyAmount, wheatAmount, wheatStorageAmount, sellWheat, farmLevel, flourAmount, flourStorageAmount, sellFlour, windmillLevel } = useGameLogicContext();
  ...
  return (
    <Box display={'flex'} flexDirection={'column'} justifyContent={'flex-start'} alignItems={'center'} border={'3px solid black'} width={'25%'} height={'100%'} padding={2} gap={2}>
      <Box display={'flex'} flexDirection={'column'} width={'100%'} border={'2px solid black'} padding={2} gap={2}>
        ...
        <CustomButton
          active={windmillWindowOpen}
          icon={<WindmillIcon size={32} color={'yellow'} />}
          onClick={() => setWindmillWindow()}
          text={'Windmill ' + ' LvL:' + windmillLevel}
        />
      </Box>
      <Box display={'flex'} flexDirection={'column'} width={'100%'} border={'2px solid black'} padding={2} gap={2}>
        ...
        <StorageSection icon={<WheatIcon size={32} color={'yellow'} />} amount={wheatAmount} maxAmount={wheatStorageAmount} sell={sellWheat} sellPrice={10} />
        <StorageSection icon={<FlourIcon size={32} color={'yellow'} />} amount={flourAmount} maxAmount={flourStorageAmount} sell={sellFlour} sellPrice={10} />
      </Box>
    </Box>
  );
}
